...
export function SettingsPage() {
  ...
  const renderFieldControl = (item: SystemConfigItem) => {
    ...
    if (!schema) {
      return (
        <textarea
          ...
          className="min-h-24 w-full rounded-lg border border-teal-900/15 bg-white px-3 py-2 text-sm text-slate-800"
        />
      )
    }
    ...
    if (schema.uiControl === 'switch' || schema.dataType === 'boolean') {
      return (
        <label className="inline-flex items-center gap-2 rounded-lg border border-teal-900/15 bg-white px-3 py-2 text-sm text-slate-700">
          <input
            type="checkbox"
    ...
    if (schema.uiControl === 'select' && schema.options?.length) {
      ...
      return (
        <select
          value={value}
          onChange={(event) => handleDraftValueChange(item.key, event.target.value)}
          className="w-full rounded-lg border border-teal-900/15 bg-white px-3 py-2 text-sm text-slate-800"
        >
          {options.map((option) => (
  ...
  return (
    <section className="space-y-[var(--dsa-layout-gap)]" data-testid="page-settings">
      <header className="rounded-2xl border border-teal-900/10 bg-white/80 p-[var(--dsa-card-padding)]">
      ...
      <div className="grid gap-[var(--dsa-layout-gap)] lg:grid-cols-[220px_minmax(0,1fr)]">
        <aside className="rounded-2xl border border-teal-900/10 bg-white/80 p-[var(--dsa-card-padding)]">
          <p className="text-xs uppercase tracking-[0.16em] text-teal-900/80">配置分类</p>
          <div className="mt-2 space-y-2">
            {categories.map((category) => {
              const count = allItems.filter((item) => (item.schema?.category || 'uncategorized') === category).length
              return (
                <button
                  key={category}
                  type="button"
                  ...
                  className={`flex w-full items-center justify-between rounded-xl border px-3 py-2 text-xs font-semibold transition ${
                    effectiveActiveCategory === category
                      ? 'border-teal-500/35 bg-teal-500/12 text-teal-900'
        ...
        <div className="space-y-[var(--dsa-layout-gap)]">
          <section className="rounded-2xl border border-teal-900/10 bg-white/80 p-[var(--dsa-card-padding)]">
            <p className="text-xs uppercase tracking-[0.16em] text-teal-900/80">当前分类配置项</p>
              ...
              {filteredItems.map((item) => (
                <article
                  ...
                  className={`rounded-xl border bg-white p-3 ${
                    issuesByKey[item.key]?.length ? 'border-rose-300/80' : 'border-teal-900/10'
                  }`}
          ...
          <section className="rounded-2xl border border-teal-900/10 bg-white/80 p-[var(--dsa-card-padding)]">
            <p className="text-xs uppercase tracking-[0.16em] text-teal-900/80">认证设置</p>
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              ...
              <div className="rounded-lg border border-teal-900/10 bg-white px-3 py-2 text-xs text-slate-600">
            ...
            <button
              ...
              className="mt-3 rounded-lg border border-teal-900/15 bg-teal-500/12 px-3 py-2 text-xs font-semibold text-teal-900 transition hover:bg-teal-500/18 disabled:opacity-60"
          ...
          <section className="rounded-2xl border border-teal-900/10 bg-white/80 p-[var(--dsa-card-padding)]">
            <p className="text-xs uppercase tracking-[0.16em] text-teal-900/80">桌面端 .env 导入导出</p>
          ...
          <section className="rounded-2xl border border-teal-900/10 bg-white/80 p-[var(--dsa-card-padding)]">
            <p className="text-xs uppercase tracking-[0.16em] text-teal-900/80">LLM 渠道连通性测试</p>
      ...
      {dirtyKeys.length > 0 ? (
        <aside
          className="fixed inset-x-4 bottom-20 z-20 rounded-xl border border-teal-900/15 bg-white/95 px-4 py-3 shadow-[0_16px_40px_rgba(15,23,42,0.15)] backdrop-blur-xl lg:inset-x-auto lg:left-[max(22rem,calc(50%-34rem))] lg:right-8 lg:bottom-6"
        ...
        >
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => void handleSaveConfig()}
              className="rounded-lg border border-teal-900/15 bg-teal-500/12 px-3 py-1.5 text-xs font-semibold text-teal-900 transition hover:bg-teal-500/18"
            >
              立即保存
            </button>
          </div>
