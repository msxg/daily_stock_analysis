...
export const analysisApi = {
  analyze: async (data: AnalysisRequest): Promise<AnalyzeResponse> => {
    ...
    const response = await apiClient.post<Record<string, unknown>>('/api/v1/analysis/analyze', requestData)
  ...
  getStatus: async (taskId: string): Promise<TaskStatus> => {
    const response = await apiClient.get<Record<string, unknown>>(`/api/v1/analysis/status/${taskId}`)
  ...
  getTasks: async (params?: { status?: string; limit?: number }): Promise<TaskListResponse> => {
    const response = await apiClient.get<Record<string, unknown>>('/api/v1/analysis/tasks', { params })
    return toCamelCase<TaskListResponse>(response.data)
  },
