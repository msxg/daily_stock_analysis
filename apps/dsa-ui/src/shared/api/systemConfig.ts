import { getParsedApiError, type ParsedApiError } from '@/shared/api/error'
...
export class SystemConfigConflictError extends Error {
  ...
  constructor(message: string, currentConfigVersion: string | undefined, parsedError: ParsedApiError) {
    super(message)
...
export const systemConfigApi = {
  ...
  async validate(payload: ValidateSystemConfigRequest): Promise<ValidateSystemConfigResponse> {
    const response = await apiClient.post<Record<string, unknown>>('/api/v1/system/config/validate', toSnakeValidatePayload(payload))
  ...
  async importDesktopEnv(payload: ImportSystemConfigRequest): Promise<UpdateSystemConfigResponse> {
    const response = await apiClient.post<Record<string, unknown>>('/api/v1/system/config/import', toSnakeImportPayload(payload))
  ...
  async update(payload: UpdateSystemConfigRequest): Promise<UpdateSystemConfigResponse> {
    try {
      const response = await apiClient.put<Record<string, unknown>>('/api/v1/system/config', toSnakeUpdatePayload(payload))
      return toCamelCase<UpdateSystemConfigResponse>(response.data)
    } catch (error: unknown) {
      ...
      if (error && typeof error === 'object' && 'response' in error) {
        ...
        if (status === 400) {
          ...
          throw new SystemConfigValidationError(parsed.message || validationError.message || '配置校验失败', validationError.issues || [], parsed)
        }

        if (status === 409) {
          const conflict = toCamelCase<SystemConfigConflictResponse>(payloadData ?? {})
          throw new SystemConfigConflictError(parsed.message || conflict.message || '配置版本冲突', conflict.currentConfigVersion, parsed)
        }
      }
