...
- 💬 **问股页面修复本地草稿会话误报权限问题** — 本地草稿会话（`local-*`）不再触发后端消息查询；仅在云端会话（`user_*` 或会话列表存在）时请求会话消息，避免每次打开问股页出现“当前账号没有该操作权限”。同时增加 403 自恢复：检测到会话权限变化会自动切换到新草稿会话。
...
- 💬 **`dsa-ui` 问股完成 Phase 2 首批能力（UI-201/UI-203）** — 问股页已打通会话列表加载、会话切换/新建草稿/删除、会话消息读取、会话头部“上下文状态条 + Markdown 导出 + 发送到通知”，并新增策略选择（下拉 + 快捷策略）与提问输入区（Enter 快捷发送）；同步补齐单元测试、集成测试、Playwright 可访问性/视觉/性能回归与真后端联调测试，确保不改后端 API 前提下交互可用性和稳定性。
- ✅ **`dsa-ui` 完成 Phase 2~Phase 6 全量交付** — 问股已完成 Markdown 渲染、思考过程折叠、SSE 流式时间线、分析台上下文注入与移动双面板；持仓完成账户/总览/风险/录入/导入/流水工作区重构；回测完成筛选、运行、KPI、趋势图与结果表；设置与登录完成分类编辑、认证、env 导入导出、首次密码设置；并新增 `build:static`/`build:preview` 切换脚本以支持接管与回滚。
- 📚 **新增 `apps/dsa-docs` 文档站点工程（Docusaurus）** — 新建独立文档项目并接入 `dsa-ui` 操作手册/重构方案/开发计划/任务追踪，统一静态资源目录与侧边栏结构；同时新增 Playwright 文档冒烟 + 视觉回归测试（含基线截图）与 `typecheck/build` 验证脚本，支持后续持续迭代与可回归交付。

### 修复

- 🗂️ **`dsa-ui` 设置页工具区按分类归位** — `UI` 分类现在仅展示 Theme 切换；管理员认证与桌面端 `.env` 导入导出收敛到“系统”分类；`LLM` 渠道连通性测试收敛到“模型与渠道”分类，不再在每个分类页面重复出现整组工具区块。
...
- 🧹 **`dsa-ui` 分析台移除写死的快捷分析按钮** — 删除分析台“历史分析”区域中预置的 `分析 600519 / AAPL / HK00700` 快捷入口，避免在未接入真实配置或个性化数据前向用户暴露固定样例项，界面也更聚焦于真实历史记录与手动输入分析流程。
- 🧭 **`dsa-ui` 主页面标题与功能简述统一上收至壳层 header** — 分析台、问股、持仓、回测、设置页已统一为“顶部 header 承载页面标题与功能简述，页面内容区仅保留操作与主体模块”的结构，移除各页面首屏重复的可见标题/说明块，减少首屏冗余占位并提升主内容区密度。
- ✂️ **`dsa-ui` 壳层装饰文案进一步精简** — 移除侧栏顶部 `Workspace`、内容头部 `DSA UI Alpha` 以及右上角 `Phase 0 Baseline` 阶段徽标，保留更聚焦的页面标题、功能简述与主导航信息，减少视觉噪音。
- 🧩 **`dsa-ui` 分析台任务卡与报告区联动修复** — 分析台左侧“任务状态”中的完成态任务现在支持直接点击定位对应报告；若当前历史列表未加载到该报告，会按股票代码补查最新历史并切换右侧报告区，避免点击任务卡后右侧内容无变化。
- 📄 **`dsa-ui` Markdown 报告区取消内部竖向滚动** — 分析台 `Markdown` 标签页中的报告内容改为随文本自然向下延伸，不再使用内部 `max-height` 滚动容器；当整体内容超出浏览器视口时，改由页面级滚动承载，提升长报告阅读连续性。
- 🎯 **`dsa-ui` 空间布局与主次信息占比优化** — 在不改后端 API 和功能点的前提下，统一收窄应用壳层左侧导航、压缩页面与模块间距、提升主内容区占比；分析台侧栏宽度从 `320px` 调整到 `280px` 并增强报告主信息展示；问股页会话区从 `320px` 收敛到 `260px` 后继续升级为顶部“会话管理”面板（移除压缩侧栏样式），并新增“当前会话标题”长度限制展示；持仓/回测/设置页同步做了卡片密度与响应式栅格优化，并新增统一布局标尺（桌面侧栏 `200px`、内容外边距 `15px`、关键模块间距 `10px`），以改善桌面与小屏可用空间利用率和浏览效率。
- 🧪 **`dsa-ui` 集成测试缓存污染修复** — 在测试基座中加入 `queryClient.clear()`，避免 React Query 缓存在用例间串扰导致的 Dashboard 批量删除场景不稳定。
- 🧰 **`dsa-ui` 测试与构建稳定性修复** — 修复聊天/登录集成测试中的路由状态串扰与 Provider 缺失问题，修复 SSE 事件联合类型导致的 TypeScript 构建失败，并修正 Playwright `workbench` 断言与路由拦截优先级问题（避免 `@visual` 场景下的误失败）。
- 📚 **新增 `dsa-ui` 操作手册（含标注截图）** — 新增 [DSA UI 操作手册](/Users/ddxx/Dev/TestWs/daily_stock_analysis/docs/dsa-ui-user-manual.md)，按功能结构覆盖分析台、问股、持仓、回测、设置、登录，并提供 18 张红框标注截图与截图再生成脚本（`npm run manual:assets`）。
- 🧭 **`dsa-docs` 操作手册结构与截图标注优化** — `apps/dsa-docs` 中的 `DSA UI 操作手册` 已升级为“单一入口 + 多页分册（导航/分析台/问股/持仓/回测/设置登录）”结构，不再单独暴露“操作手册分册”入口；同时升级截图标注脚本为“自动避让 + 引导线箭头”布局，修复滚动场景红框偏移与标签压框问题，并重生成同步 18 张手册截图资产。
...
- 💬 **`dsa-ui` 问股流式回答收口到单一底部对话流** — 发送问题后，现在会先在消息列表底部立即插入用户消息，再把“思考过程 / 取消请求 / 流式回复 / 最终答案”统一收口到同一张 AI 回复卡片中，并在流式阶段自动滚动到最新内容；不再把思考过程单独挂在页面上方，减少一次提问过程中反复上下滚动的割裂感。
- 💬 **`dsa-ui` 问股流式卡片继续收口到聊天气泡语义** — 流式中的 AI 卡片进一步贴近普通 AI 消息样式，顶部状态改为更轻量的“AI + 状态点”，输入区按钮提示从“流式中...”弱化为“发送中...”，减少页面上多个并行状态焦点，让用户把注意力放回当前这条正在生成的回复。
- 🎛️ **`dsa-ui` 问股策略下拉尺寸与快捷标签对齐** — 将问股输入区中的策略下拉框高度、字号和水平留白进一步压缩，使其与左侧快捷策略标签的视觉高度更接近，减少同一行控件尺寸不一致带来的割裂感。
